...
FaceAntiSpoofing::FaceAntiSpoofing()
{
    ...
    Json::Reader reader;
    reader.parse(configFile, _jsonObject);

    _inputName = "data";
    _outputName = "softmax";

    _ncnnOption.lightmode = true;
    _ncnnOption.num_threads = 2;
...
{
    const Json::Value &faceAntiSpoofingConfig = _jsonObject["face_anti_spoofing"];
    for (int i = 0; i < faceAntiSpoofingConfig.size(); ++i)
    {
        ...
        _ncnnNetworks.emplace_back(net);
    }
}
...
float FaceAntiSpoofing::detect(cv::Mat &frame, const cv::Rect &bbox)
{
    ...
    for (int i = 0; i < faceAntiSpoofingConfig.size(); i++)
    {
        ...
        ncnn::Extractor extractor = _ncnnNetworks[i]->create_extractor();
        extractor.set_light_mode(true);
        extractor.set_num_threads(2);

        extractor.input(_inputName.c_str(), input);
        ncnn::Mat output;
        extractor.extract(_outputName.c_str(), output);
